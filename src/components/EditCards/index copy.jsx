...
const EditCards = () => {
  ...
  useEffect(() => {
    ...
    if (deckOfId) {
      console.log('edit',deckOfId)
      setEditedDeck(deckOfId);
    }
  }, [decks, deckId]);
    ...
    <div className='main-container' style={{ backgroundImage: `url(${background})` }}>
      <div className='edit-cards-container'>
        <form onSubmit={handleEditSubmit}>
          <h2>{editedDeck && editedDeck.name && editedDeck.name.toUpperCase()}</h2>
          <h3>Edite as cartas do baralho</h3>
          <div className='card-container'>
          {editedDeck && editedDeck.cards && editedDeck.cards.map((card, index) => (
